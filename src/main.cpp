...
int main()
{
  ...
  while(running)
  {
    double before = platform_get_seconds();

    // Load the update_game function pointer from the DLL
    platform_reload_dynamic_library();

    platform_update_window();
    update_game(gameState, input, renderData, &allocator, play_sound, frameTime);
    gl_render();

    // This is platform specific!
    platform_swap_buffers();

    frameTime = platform_get_seconds() - before;
...
void update_game(GameState* gameState, Input* inputIn, RenderData* renderDataIn, BumpAllocator* allocator, PlaySoundFunc play_sound, double frameTime)
{
  update_game_ptr(gameState, inputIn, renderDataIn, allocator, play_sound, frameTime);
}